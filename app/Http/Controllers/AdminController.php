...
class AdminController extends Controller
{
    public function index(){
        $data = User::select(
            ...
            ->where('created_at', '>', Carbon::today()->subDays(6)) // Sửa thành subDays để lấy 7 ngày gần đây
    ...
    public function userPieChart(Request $request){
        ...
        $data = User::select(
            DB::raw("COUNT(*) as count"),
            DB::raw("DATE_FORMAT(created_at, '%W') as day_name"), // Sử dụng DATE_FORMAT để lấy tên ngày
            DB::raw("DAY(created_at) as day")
        )
